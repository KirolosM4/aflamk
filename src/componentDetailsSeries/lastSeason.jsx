            ...
            <p className="text-2xl py-3 text-[#0DCAF0] font-bold">Last Season</p> 
            {
                ...
                <div className="h-fit w-[90%] self-center bg-[#212529] flex flex-col text-white items-center font-bold text-base md:flex-row md:w-full md:h-[60%] md:items-start">
                    ...
                    <div className="p-5 text-center md:text-left">
                        <p className="flex flex-col gap-7 md:flex-row"><span>{lastSeason?.name}</span><span className="bg-white h-fit rounded px-2 flex gap-2 items-center text-black w-fit self-center"><IoStar className="text-[#212529]"/>{lastSeason?.vote_average}</span><span>{date.getFullYear(lastSeason?.air_date)} | {lastSeason?.episode_count} Episodes</span></p>
                        <p className="font-normal py-11">{lastSeason?.overview}</p>
                    </div>
                </div>
            }
        </div>
